...
BOOST_AUTO_TEST_CASE(TestDoubleUnique)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        auto AKeyingMaterial = A.getKeyingMaterial(B.myPublicKey);
        auto BKeyingMaterial = B.getKeyingMaterial(A.myPublicKey);

        BOOST_CHECK_EQUAL(AKeyingMaterial, BKeyingMaterial);
    }
